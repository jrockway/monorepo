...
// AddMatchRegex adds a MatchRegex to this filter scheme.  A MatchRegex filters out all lines that
// do not match it.  An empty string is a no-op.  This method may only be called with a non-empty
// string once, and returns an ErrConflict if a NoMatchRegex is set.
...
// AddNoMatchRegex adds a NoMatchRegex to this filter scheme.  A NoMatchRegex filters out all lines
// that match it.  An empty string is a no-op.  This method may only be called with a non-empty
// string once, and returns an ErrConflict if a MatchRegex is set.
